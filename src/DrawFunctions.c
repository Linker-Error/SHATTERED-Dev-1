	...
	for (y = 0; y < 20; ++y)
	{
		...
		if (y == WhereToDrawPlayer)
		{
			if (Item.IsOnTop[(int)(CharacterSprite.CurrentSprite.y)])
			{
				...
				{
					switch ((int)(CharacterSprite.CurrentSprite.y))
					{
					case 0:
						DrawSprite(Item.ItemSprite, (Vector2) { PlayerPosition.x + Item.OffSet[(int)(CharacterSprite.CurrentSprite.y)].x, PlayerPosition.y + Item.OffSet[(int)(CharacterSprite.CurrentSprite.y)].y +16 });
						break;
					case 1:
